      ...
      setFeedLogs(prev => {
        ...
        switch (step) {
          case 0:
            ...
            newLog = { id: Date.now().toString() + Math.random(), text: `Scanning Job Boards${locationText}...`, icon: <RefreshCcw className="w-4 h-4 text-secondary animate-spin" />, color: "text-secondary", time: now };
            break;
          case 1:
      ...
      <main className="max-w-[1400px] mx-auto w-full p-6 lg:p-10 flex-1">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap justify-between items-center gap-4 mb-10">
          ...
          <motion.button 
            ...
            className={`flex items-center gap-2 rounded-lg h-11 px-6 text-sm font-bold transition-all shadow-sm ${!hasUploadedResume ? 'opacity-50 cursor-not-allowed bg-slate-200 dark:bg-slate-800 text-slate-500' : isPlaying ? 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-700' : 'bg-primary text-white hover:bg-red-600'}`}
          >
            <PauseCircle size={18} />
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-5 flex flex-col gap-8">
            ...
            <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:shadow-md">
              ...
              <div 
                ...
                className={`group flex flex-col items-center gap-4 rounded-xl border-2 border-dashed transition-colors px-6 py-8 cursor-pointer relative overflow-hidden ${hasUploadedResume ? 'border-emerald-500/30 bg-emerald-500/5' : showError ? 'border-red-500 bg-red-500/10 animate-shake' : 'border-primary/30 hover:border-primary/60 bg-primary/5 hover:bg-primary/10'}`}
              >
                {!hasUploadedResume && <div className="absolute inset-0 bg-gradient-to-tr from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />}
                
                <div className="flex flex-col items-center gap-2 relative z-10 text-center">
                  <div className={`p-3 rounded-full shadow-sm mb-2 ${hasUploadedResume ? 'bg-emerald-500/20 text-emerald-500' : 'bg-white dark:bg-slate-800 text-primary'}`}>
                    {isUploading ? <Loader2 size={28} className="animate-spin" /> : hasUploadedResume ? <FileCheck2 size={28} /> : <UploadCloud size={28} />}
                  </div>
                  
                  {isUploading ? (
                    <p className="text-slate-900 dark:text-white text-lg font-bold tracking-tight">Parsing PDF...</p>
                  ) : hasUploadedResume ? (
                    <>
                      <p className="text-slate-900 dark:text-white text-lg font-bold tracking-tight">Successfully Uploaded</p>
                      <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">software_engineer_cv.pdf</p>
                    </>
                  ) : (
                    <>
                      <p className="text-slate-900 dark:text-white text-lg font-bold tracking-tight">Upload Resume</p>
                      <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Click or grab a PDF to begin</p>
                    </>
                  )}
                </div>
                {hasUploadedResume && <button className="text-emerald-600 dark:text-emerald-400 text-sm font-bold hover:underline relative z-10 mt-2">Change File</button>}
              </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-primary focus:border-primary placeholder:text-slate-400 shadow-sm transition-all" 
            ...
            <motion.button 
              whileHover={!isPlaying && { scale: 1.01, backgroundColor: "#dc2626" }}
              whileTap={!isPlaying && { scale: 0.98 }}
              onClick={handleActivate}
              disabled={isPlaying}
              className={`w-full font-black py-4 rounded-xl flex items-center justify-center gap-3 transition-all overflow-hidden relative group ${isPlaying ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : showError ? 'bg-red-600 text-white animate-shake' : 'bg-primary text-white shadow-[0_8px_30px_rgb(234,67,52,0.3)]'}`}
            >
              {!isPlaying && <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out" />}
              <Zap className={isPlaying ? "fill-slate-500 relative z-10" : "fill-white relative z-10 animate-pulse"} size={20} />
              <span className="relative z-10 tracking-wide text-sm md:text-base">{isPlaying ? "AGENT ACTIVE" : showError ? "UPLOAD RESUME FIRST" : "ACTIVATE AGENT"}</span>
            </motion.button>
          </motion.div>
